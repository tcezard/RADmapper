...
def get_offsprings_possible_genotypes_om():
    ...
    om_parents_pattern={"0/1 0/0":{"type":"D1.10", "0/0":"a", "0/1":"ab", "1/0":"ab"},
                        ...
                        "0/1 0/2":{"type":"A.2", "0/0":"a", "0/1":"ab", "0/2":"ac", "1/2":"bc"},
                        "0/1 1/2":{"type":"A.2", "1/1":"a", "0/1":"ab", "0/2":"bc", "1/2":"ac"},
                        "0/2 0/1":{"type":"A.2", "0/0":"a", "0/1":"ac", "0/2":"ab", "1/2":"bc"},
                        "0/2 1/2":{"type":"A.2", "2/2":"a", "0/1":"bc", "0/2":"ab", "1/2":"ac"},
        ...
        for vcf_records in self.reader:
            ...
            if gq_mother < self.geno_qual_threshold or gq_father < self.geno_qual_threshold:
                if gq_mother < self.geno_qual_threshold and gq_father < self.geno_qual_threshold:
                    logging.info("Bad quality in both parent: %s:%s"%(vcf_records.get_reference(),vcf_records.get_position()))
                elif gq_father < self.geno_qual_threshold:
                    logging.info("Bad quality in Father: %s:%s"%(vcf_records.get_reference(),vcf_records.get_position()))
                elif gq_mother < self.geno_qual_threshold:
                    logging.info("Bad quality in Mother: %s:%s"%(vcf_records.get_reference(),vcf_records.get_position()))
...
def phased_snps_to_joinmap(vcf_file, sex_info, mother, father, geno_qual_threshold, output_markers, output_vcf, max_missing_offspring):
    ...
    logging.info( "%s discarded because not supported by the format (%s)"%(non_supported_markers,
                                                     ', '.join(["%s:%s"%(genotype,count) for genotype,count in non_supported_genotypes.items()])))
    logging.info( "%s discarded because of bad pattern"%bad_pattern)
    


def phased_snps_to_onemap(vcf_file, sex_info, mother, father, geno_qual_threshold, output_markers, output_vcf, max_missing_offspring):
    sample_to_sex,sex_to_sample, ordered_sample = load_sex_info(sex_info)
    if mother in ordered_sample:
        ordered_sample.remove(mother)
    if father in ordered_sample:
    ...
    for geno_parents, all_offsprings_genotypes, phased_vcf_record in phased_markers:
        name = get_list_of_SNPs_name(phased_vcf_record)
        out = output_marker_in_onemap(name, possible_genotypes, ordered_sample, geno_parents, all_offsprings_genotypes,max_missing_offspring)
        if out is None:
            logging.info("Non supported genotype in parent %s for marker %s"%(geno_parents,name))
            non_supported_markers+=1
            non_supported_genotypes[geno_parents]+=1
...
def phased_snps_to_carthagene(vcf_file, sex_info, mother, father, geno_qual_threshold, output_markers_female, output_markers_male, 
    ...
    for geno_parents, all_offsprings_genotypes, phased_vcf_record in phased_markers:
        name = get_list_of_SNPs_name(phased_vcf_record)
        output_female, output_male = output_marker_in_cartagene(name, possible_genotypes, ordered_sample, geno_parents, all_offsprings_genotypes)
...
def output_marker_in_joinmap(name, possible_genotypes, ordered_samples, geno_parents, all_offsprings_genotypes, max_missing_offspring):
    ...
    if "".join(all_sample_out).count("-")/2 > max_missing_offspring:
        logging.info("nb missing offspring = %s>%s: %s"%("".join(all_sample_out).count("-")/2, max_missing_offspring, name))
        return ""
    else:
        return "%s\t%s\t%s\n"%(name, possible_genotypes.get(geno_parents).get('type'),  ' '.join(all_sample_out))

def output_marker_in_onemap(name, possible_genotypes, ordered_samples, geno_parents, all_offsprings_genotypes, max_missing_offspring):
    possible_offspring_genotypes = possible_genotypes.get(geno_parents)
    if not possible_offspring_genotypes:
        return 
    all_sample_out=[]
...
def process_genotypes_of_a_locus(phased_genotype_mother,phased_genotype_father,
    ...
    for i in range(len(samples)):
        ...
        if haplo_off1.count('.')==len(haplo_off1)  or haplo_off2.count('.')==len(haplo_off2) :
            ...
            logging.debug("For "+samples[i]+" confident haplotypes: "+haplo_off1+" "+haplo_off2+" with perfect match to "+str(allele1)+"-"+str(allele2))
            ...
            if (allele1 is None or allele2 is None) and not perfect_match :
                if allele1 != None:
                    if str(allele1) in genotype_mother and str(allele1) in genotype_father:
                        ...
                        allele2 = parents_haplotypes.get(m_haplo_off2)
                        logging.debug("For "+samples[i]+" Knowing "+str(allele1)+" is both maternal and paternal, imperfect match "+haplo_off2+" to "+str(remaining_haplo)+" resolve to "+str(allele2))
                    elif str(allele1) in genotype_mother:
                        dummy, m_haplo_off2 = match_haplotypes_to_parents([hf1,hf2], '', haplo_off2)
                        allele2 = parents_haplotypes.get(m_haplo_off2)
                        logging.debug("For "+samples[i]+" Knowing "+str(allele1)+" is maternal, imperfect match "+haplo_off2+" to ["+hf1+","+hf2+"] resolve to "+str(allele2))
                    elif str(allele1) in genotype_father:
                        dummy, m_haplo_off2 = match_haplotypes_to_parents([hm1,hm2], '', haplo_off2)
                        allele2 = parents_haplotypes.get(m_haplo_off2)
                        logging.debug("For "+samples[i]+" Knowing "+str(allele1)+" is paternal, imperfect match "+haplo_off2+" to ["+hm1+","+hm2+"] resolve to "+str(allele2))
                elif allele2 != None:
                    if str(allele2) in genotype_mother and str(allele2) in genotype_father:
                        ...
                        allele1 = parents_haplotypes.get(m_haplo_off1)
                        logging.debug("For "+samples[i]+" Knowing "+str(allele2)+" is both maternal and paternal, imperfect match "+haplo_off1+" to "+str(remaining_haplo)+" resolve to "+str(allele1))
                    elif str(allele2) in genotype_mother:
                        m_haplo_off1, dummy = match_haplotypes_to_parents([hf1,hf2], haplo_off1, '')
                        allele1 = parents_haplotypes.get(m_haplo_off1)
                        logging.debug("For "+samples[i]+" Knowing "+str(allele2)+" is maternal, imperfect match "+haplo_off1+" to ["+hf1+","+hf2+"] resolve to "+str(allele1))
...
def main():
    ...
    if options.type_output==OUTPUT_TYPE_JOINMAP:
        ...
        phased_snps_to_carthagene(options.input_vcf_file, options.sex_info_file, options.mother_name, 
                                  options.father_name, geno_qual_threshold=options.geno_qual_threshold,
                                  output_markers_female=options.output_female_marker, output_markers_male=options.output_male_marker,
...
def _verifyOption(options):
    ...
    elif (options.type_output == OUTPUT_TYPE_JOINMAP or options.type_output == OUTPUT_TYPE_ONEMAP) and not options.output_marker:
