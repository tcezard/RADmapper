...
OUTPUT_TYPE_NEXUS = 'nexus'
OUTPUT_TYPE_STRUCTURE = 'structure'
OUTPUT_TYPE_GENEPOP = 'genepop'
OUTPUT_TYPE_VCF = 'vcf'

OUTPUT_TYPE = [OUTPUT_TYPE_PHYLIP, OUTPUT_TYPE_STRUCTURE, OUTPUT_TYPE_NEXUS, OUTPUT_TYPE_GENEPOP, OUTPUT_TYPE_VCF]
...
def format_nexus(all_sequences):
    out = ['#nexus']
    out = ['begin data;']
    out.append('dimensions ntax=%s nchar=%s;' % (len(all_sequences), len(all_sequences.get(all_sequences.keys()[0]))))
    out.append('format datatype=dna missing=? gap=-;')
    out.append('matrix')
    for name in all_sequences.keys():
        out.append('%s %s' % (name, ''.join(all_sequences.get(name))))
    out.append(';')
    out.append('end;')
    return '\n'.join(out)
...
def main():
    ...
    if options.type_output == OUTPUT_TYPE_PHYLIP or options.type_output == OUTPUT_TYPE_NEXUS:
        vcf_to_phylip_or_nexus(options.input_vcf_file, options.output_file, options.pop_file,
                               options.geno_qual_threshold, options.max_prop_missing, options.type_output)
    elif options.type_output == OUTPUT_TYPE_STRUCTURE:
        vcf_to_structure(options.input_vcf_file, options.output_file, options.pop_file, options.geno_qual_threshold,
                         options.max_prop_missing, options.phased)
    elif options.type_output == OUTPUT_TYPE_GENEPOP:
        vcf_2_genepop(options.input_vcf_file, options.output_file, options.pop_file, options.geno_qual_threshold,
                      options.max_prop_missing, options.phased)
    elif options.type_output == OUTPUT_TYPE_VCF:
        vcf_2_vcf(options.input_vcf_file, options.output_file, options.geno_qual_threshold, options.max_prop_missing,
                  options.phased)
